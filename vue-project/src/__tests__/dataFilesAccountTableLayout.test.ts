...
test('公众号列表表格列宽、操作按钮和分页布局保持完整显示', () => {
  ...
  assert.match(dataFilesPageVue, /\{ title: '公众号', key: 'account', dataIndex: 'account', align: 'center'/)
  assert.doesNotMatch(dataFilesPageVue, /\{ title: '公众号', key: 'account'[^\n]*width:/)
  assert.match(nameColRule, /width:\s*auto;/)
  assert.match(timeColRule, /width:\s*104px;/)
  assert.match(sizeColRule, /width:\s*60px;/)
  assert.match(actionColRule, /width:\s*184px;/)
